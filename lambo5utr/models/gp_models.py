...
from lambo5utr.models.base_surrogate import BaseSurrogate
from lambo5utr import transforms as gfp_transforms
from lambo5utr.models.metrics import quantile_calibration
    ...
    def param_groups(self):
        ...
        if hasattr(self, "encoder") and hasattr(self.encoder, "param_groups"):
            if self.pretrain_lr is not None:
                param_groups.extend(self.encoder.param_groups(self.enc_lr, self.encoder_wd, pretrain_lr=self.pretrain_lr))
            else:
...
class MultiTaskExactGP(BaseGPSurrogate, KroneckerMultiTaskGP):
    def __init__(self,
        ...
        # initialize GP
        dummy_X = torch.randn(2, feature_dim).to(self.device).double()
        dummy_Y = torch.randn(2, out_dim).to(self.device).double()
        covar_module = (
            covar_module if covar_module is None else covar_module.to(self.device)
        )
