...
from lambo5utr import transforms as gfp_transforms, dataset as gfp_dataset
from lambo5utr.models.shared_elements import check_early_stopping
from lambo5utr.utils import str_to_tokens
...
def sample_tokens(base_tokens, logit_batch, enc_tokenizer, replacement=False, temp=1.):
	...
	# np.put_along_axis(logit_batch, non_viable_idxs, 0, axis=-1)

	if not replacement and base_tokens is not None:
		# print('base_tokens.shape:', base_tokens.shape)
		# print('logit_batch.shape:', logit_batch.shape)
		...
		# np.put_along_axis(logit_batch, base_tokens, 0, axis=-1)
...
def sample_mask(
	...
	Args:
		...
		tokenizer: need to avoid masking special tokens and to mask kmers
		mask_ratio: proportion of tokens to mask
		mask_size: (optional) override mask_ratio with a specific mask size
	Returns:
		new_mask_idxs: (batch_size, mask_size) np.ndarray of position indexes to mask
	"""
	if mask_size is None:
		mask_size = math.ceil((token_batch.shape[-1] - 2) * mask_ratio)
	...
	mask_idxs = torch.multinomial(mask_weights, mask_size, replacement=False).numpy()

	kmer_mask_idxs = mask_idxs.copy()
	# mask kmers
	for mask_num in tokenizer.mask_list:
		kmer_mask_idxs = np.concatenate([kmer_mask_idxs, mask_idxs + mask_num], axis=-1)
	# avoid masking special tokens
	for sample_idx, tmp_mask_idxs in enumerate(kmer_mask_idxs):
		tmp_nonspecial_idxs = np.where(is_non_special[sample_idx]==1)[0]
		tmp_len = len(tmp_mask_idxs)
		tmp_mask_idxs = np.intersect1d(tmp_mask_idxs, tmp_nonspecial_idxs)
		if len(tmp_mask_idxs)<tmp_len:
			tmp_mask_idxs = np.tile(tmp_mask_idxs, math.ceil(tmp_len/len(tmp_mask_idxs)))[:tmp_len]
		kmer_mask_idxs[sample_idx] = tmp_mask_idxs
	return mask_idxs, kmer_mask_idxs


def evaluate_windows(base_seqs, encoder, mask_size, replacement=True, encoder_obj='mlm'):
	window_mask_idxs = {}
	window_kmer_mask_idxs = {}
	window_entropy = {}

	for idx, seq in enumerate(base_seqs):
		window_mask_idxs[idx] = []
		window_kmer_mask_idxs[idx] = []
		window_entropy[idx] = []
		...
		offsets = np.array([offset-1, tokens.shape[-1]-1])
		for mask_start in range(offset, tokens.shape[-1] - 1, mask_size):
			...
			kmer_mask_idxs = mask_idxs.copy()
			# mask kmers
			for mask_num in encoder.tokenizer.mask_list:
				kmer_mask_idxs = np.concatenate([kmer_mask_idxs, mask_idxs + mask_num], axis=-1)
			kmer_mask_idxs = np.setdiff1d(kmer_mask_idxs, offsets)
			kmer_mask_idxs = np.tile(kmer_mask_idxs, encoder.tokenizer.kmer)[:encoder.tokenizer.kmer].reshape(1,-1)

			with torch.no_grad():
				masked_inputs = tokens.clone().to(encoder.device)
				np.put_along_axis(masked_inputs, kmer_mask_idxs, encoder.tokenizer.masking_idx, axis=1)
				...
				window_kmer_mask_idxs[idx].append(kmer_mask_idxs.copy())
				window_entropy[idx].append(logit_entropy.mean().item())

	return window_mask_idxs, window_kmer_mask_idxs, window_entropy
...
def mlm_train_step(model, optimizer, token_batch, mask_ratio, loss_scale=1.):
	...
	mask_idxs, kmer_mask_idxs = sample_mask(token_batch, model.tokenizer, mask_ratio)
	masked_token_batch = token_batch.clone().to(model.device)
	np.put_along_axis(masked_token_batch, kmer_mask_idxs, model.tokenizer.masking_idx, axis=1)
	...
	masked_logits = np.take_along_axis(logits, kmer_mask_idxs[..., None], axis=1).view(-1, vocab_size)

	# use the ground-truth tokens as labels
	# -100 is the default ignore_index value for F.cross_entropy()
	# ignore_matrix = torch.full(token_batch.size(), -100).to(model.device)
	# masked_label_batch = torch.where(masked_token_batch==model.tokenizer.masking_idx, token_batch.to(model.device), ignore_matrix).view(-1)

	# use the ground-truth tokens as labels
	# masked_tokens = np.take_along_axis(token_batch, mask_idxs, axis=1)
	masked_tokens = np.take_along_axis(token_batch, kmer_mask_idxs, axis=1)
	masked_tokens = masked_tokens.view(-1).to(model.device)

	# loss = loss_scale * F.cross_entropy(logits, masked_label_batch)
	loss = loss_scale * F.cross_entropy(masked_logits, masked_tokens)
	loss.backward()
	optimizer.step()

	# return loss, logits, masked_label_batch
...
def mlm_train_epoch(model, optimizer, train_loader, mask_ratio):
	...
	for minibatch in train_loader:
		...
		# logging
		# perplexity = torch.exp(F.cross_entropy(logits, masked_label_batch))
		# metrics['train_perplexity'] += perplexity.detach() / len(train_loader)
...
def mlm_eval_epoch(model, eval_loader, mask_ratio, split):
	...
	for minibatch in eval_loader:
		...
		# logging
		# perplexity = torch.exp(F.cross_entropy(logits, masked_label_batch))
		# metrics['perplexity'] += perplexity.detach() / len(eval_loader)
