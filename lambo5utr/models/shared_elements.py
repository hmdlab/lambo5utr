...
from lambo5utr.models.lm_elements import PositionalEncoding, FunctionHead, LengthHead, LengthTransform
...
class BaseEncoder(nn.Module):
    def __init__(self, tokenizer, max_seq_len=None, max_len=None, **kwargs):
        super().__init__(**kwargs)
        self.tokenizer = tokenizer
        if max_len is None and max_seq_len is not None:
            self.max_len = max_seq_len - self.tokenizer.kmer + 1 + 2
        elif max_len is not None:
            self.max_len = max_len
    
    def enc_tok_features(self):
        raise NotImplementedError

    def dec_tok_features(self):
        raise NotImplementedError

    def tgt_tok_logits(self):
        raise NotImplementedError
...
class mCNN(BaseEncoder):
    ...
    def __init__(self, embed_dim=128, kernel_size=5, p=0.1, out_dim=1,
        ...
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=self.tokenizer.padding_idx)
        self.pos_encoder = PositionalEncoding(embed_dim, p, self.max_len, batch_first=True)
...
class Transformer(BaseEncoder):
    ...
    def __init__(self, embed_dim=64, ff_dim=256, num_heads=2, num_layers=4, p=0.1, out_dim=1,
