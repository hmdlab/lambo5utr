...
from lambo5utr import utils
# from lambo5utr.tasks.chem.logp import prop_func
from lambo5utr.models.mlm import sample_tokens
...
def safe_vocab_mutation(tokenizer, problem, cand_idx, res_idx):
    ...
    for seq, idx in zip(seqs, res_idx):
        tokens = tokenizer.decode_lambo(tokenizer.encode_lambo(seq)).split(" ")[1:-1]
        safe_mut = None
        for i in range(50):
...
class LocalMutation(Mutation):
    ...
    def _do(self, problem, x, **kwargs):
        ...
        for i, idx in enumerate(x0):
            num_tokens = len(self.tokenizer.encode_lambo(problem.candidate_pool[idx].mutant_residue_seq)) - 2
            x1[i] = min(num_tokens - 1, x1[i])
            # TODO always work with token indices?
            # num_tokens = len(self.tokenizer.encode_lambo(cand_seq))
