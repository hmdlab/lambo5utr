...
from lambo5utr.optimizers.mutation import get_mlm_mutation


def _draw_samples(tokenizer, cand_pool, problem, num_samples, mlm_obj=None):
    ...
    x1 = []
    for idx in x0:
        num_tokens = len(tokenizer.encode_lambo(cand_pool[idx].mutant_residue_seq)) - 2
        x1.append(np.random.randint(0, num_tokens))
        # TODO always work with token indices?
        # num_tokens = len(tokenizer.encode_lambo(cand_pool[idx].mutant_residue_seq))
