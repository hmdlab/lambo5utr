...
from lambo5utr.models.mlm import sample_tokens, evaluate_windows
from lambo5utr.optimizers.pymoo import pareto_frontier, Normalizer
from lambo5utr.models.shared_elements import check_early_stopping
from lambo5utr.utils import (
    weighted_resampling,
    DataSplit,
    update_splits,
    str_to_tokens,
...
from lambo5utr.models.lanmt import corrupt_tok_idxs
import logging
...
class LaMBO(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.prune_check_rounds = round(num_rounds * 0.25)
    ...
    def optimize(
        ...
        if self.ref_point_method == "nadir":
            logger.info("-- nadir method --")
            self._ref_point = -infer_reference_point(
                -torch.tensor(norm_pareto_targets)
            ).numpy()
            logger.info(f"reference point: {self._ref_point}")
            logger.info(
                f"rescaled reference point: {hypercube_transform.inv_transform(self._ref_point)}"
            )
            hv_indicator = get_performance_indicator("hv", ref_point=self._ref_point)
            new_hypervol = hv_indicator.do(norm_pareto_targets)
            logger.info(f"abs_hv: {new_hypervol}")
        elif self.ref_point_method == "median":
            logger.info("-- median method --")
            self._ref_point = - hypercube_transform(
                np.median(all_targets, axis=0)
            ).copy()
            logger.info(f"reference point: {self._ref_point}")
            logger.info(
                f"rescaled reference point: {hypercube_transform.inv_transform(self._ref_point)}"
            ...
            new_hypervol = hv_indicator.do(norm_pareto_targets)
            logger.info(f"abs_hv: {new_hypervol}")
        elif self.ref_point_method == "hypercube_loc":
            logger.info("-- hypercube loc method --")
            self._ref_point = - hypercube_transform(
                (target_min + 0.5 * target_range)
            ).copy()
            logger.info(f"reference point: {self._ref_point}")
            logger.info(
                f"rescaled reference point: {hypercube_transform.inv_transform(self._ref_point)}"
            ...
            new_hypervol = hv_indicator.do(norm_pareto_targets)
            logger.info(f"abs_hv: {new_hypervol}")
        elif self.ref_point_method == "hypercube_and_nadir":
            logger.info("-- min of hypercube loc and nadir method --")
            hypercube_ref_point = hypercube_transform(
                (target_min + 0.5 * target_range)
            ).copy()
            nadir_ref_point = infer_reference_point(
                -torch.tensor(norm_pareto_targets)
            ).numpy()
            self._ref_point = - np.min(np.stack([hypercube_ref_point, nadir_ref_point], -1), -1)
            logger.info(f"reference point: {self._ref_point}")
            logger.info(
                f"rescaled reference point: {hypercube_transform.inv_transform(self._ref_point)}"
            )
        ...
        if trial is not None:
            trial.report(
                metrics["/".join([log_prefix, "opt_metrics", "hypervol_rel"])], 0
            )
        ...
        logger.info(pd.DataFrame([obj_vals]).to_markdown(floatfmt=".4f"))
        ...
        for round_idx in range(1, self.num_rounds + 1):
            ...
            if (
                ...
                logger.info(
                    f"active set contracted to {self.active_candidates.shape[0]} pareto points"
                )
            ...
            # resampling_temp: bias examples towards "good" ones
            ...
            logger.info(pd.DataFrame([best_entry]).to_markdown(floatfmt=".4f"))
            metrics.update(
                dict(
            ...
            for gen_idx in range(self.num_gens):
                ...
                tgt_lens = (
                    None
                    if self.bb_task.allow_len_change and self.encoder_obj=="lanmt"
                    else base_lens
                )

                with torch.no_grad():
                    window_mask_idxs, window_kmer_mask_idxs, window_entropy = evaluate_windows(
                        base_seqs,
                        self.encoder,
                        self.window_size,
                        replacement=True,
                ...
                if self.position_sampler == "entropy_method":
                    ...
                    mask_idxs = None
                    kmer_mask_idxs = None
                    if self.mutation_method == "one_per_seq":
                        num_mutations = 1
                    elif self.mutation_method == "mask_annealing":
                        if round_idx > self.annealing_epochs:
                            num_mutations = 1
                        else:
                            max_mutations = int(
                                self.bb_task.max_seq_len * self.mask_ratio
                            )
                            logger.debug(f"max_mutations:{max_mutations}")
                            num_mutations = (
                                max_mutations - int((max_mutations - 1) * (round_idx-1) / self.annealing_epochs)
                            )
                            logger.debug(f"num_mut:{num_mutations}")

                    for w_idxs, kmer_w_idxs in zip(window_mask_idxs.values(), window_kmer_mask_idxs.values()):
                        tmp_idxs = None
                        kmer_tmp_idxs = None
                        for _ in range(num_mutations):
                            random_idx = random.randint(0, len(w_idxs) - 1)
                            if tmp_idxs is None:
                                tmp_idxs = w_idxs[random_idx]
                                kmer_tmp_idxs = kmer_w_idxs[random_idx]
                            else:
                                tmp_idxs = np.concatenate(
                                    [tmp_idxs, w_idxs[random_idx]], axis=1
                                )
                                kmer_tmp_idxs = np.concatenate(
                                    [kmer_tmp_idxs, kmer_w_idxs[random_idx]], axis=1
                                )
                                logger.debug(f"tmp_idx:{tmp_idxs}")
                                logger.debug(f"kmer_tmp_idx:{kmer_tmp_idxs}")

                        if mask_idxs is None:
                            mask_idxs = tmp_idxs
                            kmer_mask_idxs = kmer_tmp_idxs
                        else:
                            mask_idxs = np.concatenate([mask_idxs, tmp_idxs], axis=0)
                            kmer_mask_idxs = np.concatenate(
                                [kmer_mask_idxs, kmer_tmp_idxs], axis=0
                            )
                else:
                    raise ValueError
                ...
                with torch.no_grad():
                    ...
                    if self.latent_init == "perturb_pareto":
                        ...
                        orig_src_tok_idxs = src_tok_idxs.clone()
                        np.put_along_axis(
                            src_tok_idxs,
                            kmer_mask_idxs,
                ...
                # logger.info("generation:", gen_idx, "opt_params.device:", opt_params.device)
                ...
                for step_idx in range(self.num_opt_steps):
                    if self.encoder_obj == "lanmt":
                        ...
                        tgt_tok_idxs = orig_src_tok_idxs.clone()
                        
                        # kmer_mask method
                        # e.g.) ATG, TGC, GCC, CCA, CAT => ATG, mask, mask, mask, CAT 
                        if self.replacement_method == "kmer_mask": # default: normal
                            new_tok_idxs = np.take_along_axis(
                                new_tok_idxs, kmer_mask_idxs, axis=1
                            )
                            np.put_along_axis(
                                tgt_tok_idxs, kmer_mask_idxs, new_tok_idxs, axis=1
                            )
                            logit_entropy = np.take_along_axis(
                                logit_entropy, kmer_mask_idxs, axis=1
                            )
                        else:
                            new_tok_idxs = np.take_along_axis(
                            ...
                            np.put_along_axis(
                                tgt_tok_idxs, mask_idxs, new_tok_idxs, axis=1
                            )
                            logit_entropy = np.take_along_axis(
                    ...
                    if self.replacement_method == "mask_dominant":
                        tgt_seqs = tokens_to_str(
                            tgt_tok_idxs, self.encoder.tokenizer, mask_idxs
                        )
                    else:
                        tgt_seqs = tokens_to_str(
                            tgt_tok_idxs, self.encoder.tokenizer
                        )
            ...
            logger.info(pd.DataFrame([metrics]).to_markdown(floatfmt=".4f"))
            metrics = {
                "/".join((log_prefix, "opt_metrics", key)): val
                for key, val in metrics.items()
            }
            ...
            logger.info("---- querying objective function ----")
            ...
            for b_cand, n_cand, f_val in zip(
                ...
                logger.info(f"{len(b_cand)} --> {len(n_cand)}: {f_val}")
            ...
            if "hypervol_rel" in metrics.keys():
                last_metrics = metrics
            
            prune_flg = False
            if trial is not None:
                trial.report(
                    metrics["/".join([log_prefix, "opt_metrics", "hypervol_rel"])],
                    round_idx,
                )
                if (
                    round_idx != 0
                    and round_idx % self.prune_check_rounds == 0
                    and trial.should_prune()
                ):
                    prune_flg = True
                    break

        self.candidate_logger = wandb.Table(data=self.candidate_logger)
        wandb.log({"/".join((log_prefix, "candidates")):self.candidate_logger})
        return last_metrics, prune_flg
    ...
    def _log_candidates(self, candidates, targets, round_idx, log_prefix):
        ...
        if isinstance(self.candidate_logger, type(None)):
            self.candidate_logger = pd.DataFrame(columns=table_cols) # wandb.Table(columns=table_cols)
        for cand, obj in zip(candidates, targets):
            new_row = [
                round_idx,
                cand.uuid,
                cand.wild_name,
                str(cand.mutant_residue_seq),
            ]
            new_row.extend([elem for elem in obj])
            new_row = pd.Series(new_row, index=self.candidate_logger.columns)
            self.candidate_logger = self.candidate_logger.append(
