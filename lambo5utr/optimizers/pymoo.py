...
from lambo5utr.tasks.surrogate_task import SurrogateTask
from lambo5utr.models.lm_elements import LanguageModel
from lambo5utr.utils import weighted_resampling, DataSplit, update_splits, safe_np_cat
...
class SequentialGeneticOptimizer(object):
    ...
    def optimize(self, candidate_pool, pool_targets, all_seqs, all_targets, log_prefix=''):
        ...
        self.candidate_logger = wandb.Table(dataframe=self.candidate_logger)
        wandb.log({'/'.join((log_prefix, 'candidates', 'table')): self.candidate_logger})
        return metrics
    ...
    def _log_candidates(self, candidates, targets, round_idx, log_prefix):
        ...
        for cand, obj in zip(candidates, targets):
            new_row = [round_idx, cand.uuid, cand.wild_name, str(cand.mutant_residue_seq)]
            new_row.extend([elem for elem in obj])
            new_row = pd.Series(new_row, index=self.candidate_logger.columns)
            self.candidate_logger = self.candidate_logger.append(new_row, ignore_index=True)
...
class ModelBasedGeneticOptimizer(SequentialGeneticOptimizer):
    ...
    def _create_inner_task(self, candidate_pool, candidate_weights, input_data, target_data, transform, ref_point,
        ...
        inner_task = SurrogateTask(self.tokenizer,
                                candidate_pool,
                                acq_fn,
